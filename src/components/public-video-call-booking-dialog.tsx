...
import { AlertCircle, CalendarDays, CheckCircle2, Clock3, Loader2, Trash2, Video } from "lucide-react";
...
type BookingFormState = { phoneNumber: string };
const DEFAULT_FORM: BookingFormState = { phoneNumber: "" };
...
export function PublicVideoCallBookingDialog({
  ...
  useEffect(() => {
    if (!open || booking || meetingStatus !== "available" || !selectedDate) return;
    let cancelled = false;
    const load = async () => {
